...
public class Finca implements Serializable
{
	...
	public void cargarLotesCSV(String rutaLotesCSV)
	{
		...
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				...
				nuevoLoteCSV(linea[1], linea[2], Double.parseDouble(linea[3]), Double.parseDouble(linea[4]));
	...
	public void cargarEmpleadosCSV(String rutaEmpleadosCSV)
	{
		...
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				...
				nuevoEmpleadoCSV( linea[1], linea[2],fecha, linea[4], linea[5], linea[6],linea[7], Double.parseDouble(linea[8]),Double.parseDouble(linea[9]));
	...
	public void cargarMaquinasCSV(String rutaMaquinasCSV)
	{
		...
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				...
				nuevaMaquinaCSV(linea[1], Integer.parseInt(linea[2]),Double.parseDouble(linea[3]));
			}
	...
	public void cargarInsumosCSV(String rutaInsumosCSV)
	{
		...
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				...
				nuevoInsumoCSV(linea[1], Double.parseDouble(linea[2]), Double.parseDouble(linea[3]), linea[4]);
	...
	{
		proovedores=new ArrayList<Proovedor>();
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				nuevoProovedorCSV(linea[1],linea[2]);
			}
		}
		catch (Exception e) 
			{
	...
	public void cargarCultivosCSV(String rutaCultivosCSV)
	{
		try
		{
			...
			while((linea=csvReader.readNext())!=null)
			{
				...
				double produccionKG=Double.parseDouble(linea[9]);
				System.out.println(produccionKG);
				Cultivo cultivo=nuevoCultivoCSV(linea[0], linea[7], linea[1], fechaSiembra, fechaCorta, servicios,produccionKG);
	...
	public void salvarFinca() throws Exception
{
	try
	{
		String rutaSource="./src/main/resources";
		
		
		String rutaLotesFinal=rutaSource+"/data/lotesFinal.csv";
		salvarLotesCSV(rutaLotesFinal);
		
		
		String rutaEmpleadosFinal=rutaSource+"/data/empleadosFinal.csv";
		salvarEmpleadosCSV(rutaEmpleadosFinal);
		
		String rutaMaquinasFinal=rutaSource+"/data/maquinasFinal.csv";
		System.out.print("Mquinas Salvar");
		salvarMaquinasCSV(rutaMaquinasFinal);
		
		String rutaInsumosFinal=rutaSource+"/data/insumosFinal.csv";
		salvarInsumosCSV(rutaInsumosFinal);
		
		String rutaProovedoresFinal=rutaSource+"/data/proovedoresFinal.csv";
		salvarProovedoresCSV(rutaProovedoresFinal);
		
		String rutaServiciosFinal=rutaSource+"/data/serviciosFinal.csv";
		salvarServiciosCSV(rutaServiciosFinal);
		System.out.print("Servicios Salvar");
		
		String rutaComprasFinal=rutaSource+"/data/comprasFinal.csv";
		salvarComprasCSV(rutaComprasFinal);
		
		String rutaCultivoFinal=rutaSource+"/data/cultivosFinal.csv";
		salvarCultivoCSV(rutaCultivoFinal);
		System.out.print("Cultivo Salvar");
		
		//PROPERTIES!!!!!!! 
		File f = new File(rutaSource+"/data/FlorenciaProperties.properties");
	...
	public void salvarMaquinasCSV(String rutaMaquinasFinal) 
	{
		try
		{
			...
			CSVWriter csvWriter = new CSVWriter(writer,
	                ...
	                CSVWriter.DEFAULT_ESCAPE_CHARACTER,
	                CSVWriter.DEFAULT_LINE_END);
			String[] header={"ID_0","NOMBRE_1","DIAS_2","HOROMETRO_3"};
			...
			for(Maquina iMaquina:maquinas)
			{
				...
				String[] iM={iMaquina.darID(),iMaquina.darNombre(),Integer.toString(iMaquina.darHorasTrabajo()),Double.toString(iMaquina.darHorometro())};
	...
	public void salvarServiciosCSV(String rutaServiciosFinal)
	{
		try
		{
			...
			for(Servicio iServicio:servicios)
			{
				...
				String[] iS={iServicio.darID(),formattedString,iServicio.darTipo(),iServicio.darLote().darID(),iServicio.darCultivo().darID(),maquinasLinea,empleadosLinea,insumosDosisLinea,Double.toString(iServicio.darCostoUnidad()) };
	...
	public void salvarCultivoCSV(String rutaCultivoFinal) 
	{
		try
		{
			...
			CSVWriter csvWriter = new CSVWriter(writer,
	                ...
	                CSVWriter.DEFAULT_ESCAPE_CHARACTER,
	                CSVWriter.DEFAULT_LINE_END);
			String[] header={"ID_0","PRODUCTO_1","SIEMBRA_FECHA_2","CORTA_FECHA_3","SERVICIOS_4","CULTIVO_ANTERIOR_5","CULTIVO_SIGUIENTE_6","LOTE_7","PRODUCCION_8","PRODUCCION_KG_9"};
			csvWriter.writeNext(header);
			for(Lote iLote:lotes)
			{
				...
				for(Cultivo iCultivo:cultivos)
				{
					...
					String[] iC={iCultivo.darID(),iCultivo.darProducto(),
							...
							Double.toString(iCultivo.darProduccionInsumo())};
					csvWriter.writeNext(iC);
				}
			}
			csvWriter.close();
	...
	{
		ArrayList<Cultivo> resp=new ArrayList<Cultivo>();
	...
	public void nuevoEmpleadoCSV(String nNombre, String nCedula,LocalDate nFecha,String nEps,String zapatos
	...
	public void nuevaMaquinaCSV(String nNombre,int nHoras,double horometro)
	{
	...
	public void nuevoLoteCSV(String nNombre, String nUbicacion, double nCoste, double nArea)
	{
	...
	public void nuevoServicioCSV(LocalDate nFecha, String nTipo, Lote lote, String[] nMaquinas, String[] nEmpleados,String[] nInsumos, double nCostoXArea)
	{
		...
		//maquinas
		ArrayList<Maquina> maquinasUsadas=new ArrayList<Maquina>();
		...
		ArrayList<Empleado> empleadosUsados=new ArrayList<Empleado>();
		...
		ArrayList<Insumo> insumosUsados=new ArrayList<Insumo>();
		...
		for (int i = 0; i < nInsumos.length; i++) 
		{
			...
			if(iInsumo.darNombre().startsWith("PROD"))
			{
				esCarga=true;
			}
		...
		Servicio servicio=new Servicio(esCarga,servicios.size(),nFecha, nTipo, lote,lote.darCultivoActual(), maquinasUsadas, empleadosUsados,insumosUsados, nCostoXArea);
	...
	public void nuevoServicio(LocalDate nFecha, String nTipo, Lote lote, String[] nMaquinas, String[] nEmpleados,String[] nInsumos, double nCosto)
	{
		double areaLote=lote.darArea();
		//maquinas
		ArrayList<Maquina> maquinasUsadas=new ArrayList<Maquina>();
		...
		boolean esCarga=false;
			...
			{
				String[] iS=nInsumos[i].split("[|]");
				System.out.println(iS[0].startsWith("PROD"));
				if(iS[0].startsWith("PROD"))
				{
					esCarga=true;
					System.out.println(iS[0]);
					int indeInsu=buscarInsumoIndex(iS[0]);
					System.out.println(indeInsu);
					//esta compra se registra dos veces, el transporte y el almacenamiento
					double cantidadTotal=Double.parseDouble(iS[1]);
					double cantidadRepeticion=cantidadTotal/2;
					insumos.get(indeInsu).registrarCompra(cantidadRepeticion, lotes.get(0).darNombre());
					Insumo iInsumo=insumos.get(indeInsu);
					insumosUsados.add(new Insumo(Integer.parseInt(iInsumo.darID().substring(2)),iInsumo.darNombre(), cantidadTotal, iInsumo.darValorUnidad(), iInsumo.darTipoMedida(),lotes.get(0).darNombre()));
		...
		Servicio servicio=new Servicio(esCarga,servicios.size(),nFecha, nTipo, lote,lote.darCultivoActual(), maquinasUsadas, empleadosUsados,insumosUsados, nCosto);
		lote.darCultivoActual().agregarServicio(servicio);
		for(int m=0;m<maquinasUsadas.size();m++)
		{
			Maquina mMaq=maquinasUsadas.get(m);
	...
	public Cultivo nuevoCultivoCSV(String IDCultivo,String IDLote, String nProducto, LocalDate nPeriodo, LocalDate corta,String[] serviciosID,double produccionKG)
	{
		
		Lote lote=buscarLoteID(Integer.parseInt(IDLote.substring(2)));
		
		Cultivo cultivo=new Cultivo(this,Integer.parseInt(IDCultivo.substring(2)),nProducto, nPeriodo,lote,serviciosID,produccionKG);
	...
	public void nuevoCultivo(String nNombre, String nProducto, LocalDate nPeriodo, LocalDate corta)
	{
		...
		Cultivo cultivo=new Cultivo(this,(ultimoIndexCultivo+1),nProducto, nPeriodo,lote,null,-1);
		lote.actualizarCultivoActual(cultivo,corta);
		ultimoIndexCultivo++;
	}
	public Insumo nuevoInsumoCSV(String nNombre,double nCantidad,double nValorUnidad,String nTipoMedida)
	{
		Insumo rta=null;
		...
		{
			insumos.set(buscarInsumoIndex(nNombre), insu);
		}
		return rta;
	}
	public Insumo nuevoInsumo(String nNombre,double nCantidad,double nValorUnidad,String nTipoMedida)
	{
		Insumo rta=null;
		//se asume que la ubicacion es la florencia
		if(existeInsumo(nNombre)==false)
	...
	public boolean nuevoProovedorCSV(String nNombre ,String nNIT) 
	{
		boolean rta=false;
	...
	public Object[] generarReporteCierre()
	{
		...
		ArrayList<Insumo> reporteInsumos= new ArrayList<Insumo>();
		ArrayList<String> proovedorReporte=new ArrayList<String>();
		ArrayList<Double> proovedorDeudaReporte=new ArrayList<Double>();
